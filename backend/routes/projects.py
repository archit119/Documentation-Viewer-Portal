...
def get_project(project_id):
    ...
    try:
        ...
        # Debug: Log the metadata being returned
        print(f"🔍 Returning project {project_id}")
        print(f"🔍 Project generation_metadata type: {type(project.generation_metadata)}")
        print(f"🔍 Project generation_metadata content: {project.generation_metadata[:200] if project.generation_metadata else 'None'}...")
        
        project_dict = project.to_dict()
        print(f"🔍 to_dict() generation_metadata: {project_dict.get('generation_metadata', 'Missing key')}")
...
def update_documentation_section(project_id):
    ...
    try:
        ...
        print(f"🔧 Updating section {section_id} for project {project_id}")
        print(f"🔧 Current metadata: {project.generation_metadata}")
        ...
        print(f"🔧 New metadata: {json.dumps(current_metadata)}")
        ...
        updated_project = Project.get_by_id(project_id, request.current_user.id)
        print(f"🔧 Verified metadata after update: {updated_project.generation_metadata}")
