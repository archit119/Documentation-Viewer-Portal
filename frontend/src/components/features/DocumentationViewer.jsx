...
// Enhanced function to extract headings from HTML content for sidebar
...
// Enhanced Right sidebar with auto-detected headings
const SubsectionSidebar = ({ content, activeSubsectionId, onSubsectionSelect }) => {
  const [headings, setHeadings] = useState([]);

  useEffect(() => {
    if (!content) return;
    setHeadings(extractHeadingsFromContent(content));
  }, [content]);
...
function insertSectionDivider() {
  ...
  // Create orange divider HTML
  const dividerHTML = `
    <div class="my-8 flex items-center">
      <div class="flex-1 h-0.5 bg-gradient-to-r from-transparent via-orange-500 to-transparent"></div>
      <div class="px-4">
  ...
  try {
    ...
    const dividerElement = tempDiv.firstElementChild;
    
    range.insertNode(dividerElement);
    
    // Position cursor after the divider
    const newRange = document.createRange();
    newRange.setStartAfter(dividerElement);
    ...
    console.error('Failed to insert divider:', error);
  }
}
...
chips.forEach(chip => {
  ...
  const [tabsModified, setTabsModified] = useState(false);
  ...
  // 2) merge saved per-section edits from metadata (if any)
try {
  console.log('🔍 Raw generation_metadata:', project.generation_metadata);
  
  // The metadata might be double-encoded or have a different structure
  let metadata = project.generation_metadata;
  ...
  // Check if it has nested structure from backend
  if (metadata && typeof metadata === 'object') {
    console.log('🔍 Parsed metadata keys:', Object.keys(metadata));
    
    // Look for section_updates in the root or nested
    const updates = metadata.section_updates || 
                   (metadata.sections_updates) || 
                   (metadata.updates) || 
                   {};
                   
    console.log('🔄 Found section updates:', Object.keys(updates));
    console.log('🔄 Section updates content:', updates);
    
    parsedSections = parsedSections.map(s => {
      if (updates[s.id]) {
        console.log(`✅ Applying saved edit to section: ${s.id}`);
        return { ...s, fullContent: updates[s.id].content };
      }
      return s;
    });
  } else {
    console.warn('🔍 Metadata is not an object:', typeof metadata, metadata);
  }
} catch (e) {
  console.warn('Failed to parse generation_metadata:', e);
  console.warn('Raw metadata value:', project.generation_metadata);
}
  ...
  setIsLoading(false);
}, [project?.id, project?.documentation, project?.generation_metadata]);
...
const handleDeleteTab = (tabId) => {
  const updatedSections = sections.filter(section => section.id !== tabId);
  setSections(updatedSections);
  setTabsModified(true);
  
  // If deleted tab was active, switch to first available tab
  if (activeSectionId === tabId && updatedSections.length > 0) {
...
const handleAddTab = (title) => {
  ...
  const newSection = {
    ...
    subsections: [],
    content: '',
    fullContent: `<!--HTML_DOC--><h1>${title}</h1><p>Start editing this section...</p>`
  };
...
const handleSaveAllTabs = async (sectionsArg) => {
  // Use latest sections if provided (prevents stale state when called right after setSections)
  const currentSections = sectionsArg || sections;

  if (!tabsModified) return;
  ...
  try {
    ...
    const response = await fetch(`http://localhost:5000/api/projects/${project.id}/documentation`, {
      ...
      body: JSON.stringify({ 
        ...
        tabs_data: currentSections.map(s => ({
          id: s.id,
          title: s.title,
          icon: s.icon
        }))
    ...
    if (data.success) {
      setTabsModified(false);
      alert('All tabs saved successfully!');
    } else {
      throw new Error(data.error || 'Failed to save tabs');
    }
  } catch (error) {
    ...
    if (data.success) {
      ...
      const updatedSections = sections.map(section =>
        section.id === activeSectionId
          ? { ...section, fullContent: newSectionContent }
          : section
      );
      ...
      // Force update the project object to ensure metadata is current
      setProject(prev => ({
        ...prev,
        generation_metadata: data.updated_metadata || prev.generation_metadata,
...
const filteredSections = q
  ? sections.filter(s =>
      s.id === activeSectionId ||                            // <— keep the open tab visible
      s.title.toLowerCase().includes(q) ||
      s.fullContent.toLowerCase().includes(q)
    )
  : sections;
      ...
      <p className="text-sm text-white/70 mt-1">
        {activeTab === 'documentation' 
          ...
          : 'Visual representations and architectural diagrams'
            ...
            {/* Tab Management Button - Admin Only */}
            {isAdmin && !isEditing && (
              <button
  ...
  // 2) Update local state for the active section (UI stays in sync)
  const updatedSections = sections.map(section =>
    section.id === activeSectionId
      ? { ...section, fullContent: sectionReplacement }
      : section
  );
  setSections(updatedSections);
  ...
  // 4) Always persist ONLY this section when clicking "Save" in edit mode
  const result = await handleSaveDocumentation(sectionReplacement);
  console.log('💾 Save result:', result);

  // 5) Exit edit mode and restore scroll position
  setIsEditing(false);
  ...
  >
    ...
    <div className="max-w-4xl">
      <div id={`section-${activeSection.id}`} className="scroll-mt-8">
        ...
        {/* Right Sidebar - Auto-detected Navigation - Only show for documentation tab */}
        {activeSection && (
          <SubsectionSidebar
            content={activeSection.fullContent}
    ...
    <div className="h-full">
      ...
      {tabsModified && (
        <div className="fixed bottom-4 right-4 z-50">
          <div className="bg-orange-500/90 backdrop-blur-xl border border-orange-500/50 rounded-xl p-4 shadow-2xl">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="text-sm text-white/80">You have modified tabs</div>
              </div>
              <button
...
// Tab Management Modal Component
const TabManagementModal = ({ isOpen, onClose, sections, onDeleteTab, onAddTab, onReorderTabs }) => {
  ...
  const handleDeleteConfirm = (tabId) => {
    onDeleteTab(tabId);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-2xl bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/20">
          <h2 className="text-xl font-bold text-white">Manage Tabs</h2>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-3">
            <h3 className="text-lg font-semibold text-white">Add New Tab</h3>
            <div className="flex space-x-3">
              <input
                ...
                className="flex-1 px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-orange-500/50"
                onKeyPress={(e) => e.key === 'Enter' && handleAddTab()}
              />
              <button
                onClick={handleAddTab}
                disabled={!newTabTitle.trim()}
                className="px-4 py-2 bg-orange-500/20 text-orange-300 rounded-lg hover:bg-orange-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed border border-orange-500/30"
              >
                Add Tab
              </button>
            </div>
          ...
          {/* Existing Tabs */}
          <div className="space-y-3">
            <h3 className="text-lg font-semibold text-white">Existing Tabs ({sections.length})</h3>
              ...
              {sections.map((section, index) => (
                <div key={section.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/10">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className="font-medium text-white">{index + 1}. {section.title}</div>
          ...
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-xl p-6 max-w-md">
            ...
            <div className="flex space-x-3">
              <button
                ...
                className="flex-1 px-4 py-2 bg-red-600/20 text-red-300 rounded-lg hover:bg-red-600/30 transition-colors border border-red-600/40"
              >
                Delete
              </button>
              <button
